...
export var GenerateFromImageWithTextAcceptEnum;
...
})(GenerateFromImageWithTextAcceptEnum || (GenerateFromImageWithTextAcceptEnum = {}));
/**
 ...
 */
export function version1ImageGenerateFromImageWithText(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.GenerateFromImageWithTextRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    else if (isReadableStream(payload.ImageToImageRequestBody.init_image.content)) {
        const buffer = await readableStreamToArrayBuffer(payload.ImageToImageRequestBody.init_image.content);
        const blob = new Blob([buffer], { type: "application/octet-stream" });
        appendForm(body, "init_image", blob);
    }
    else {
        appendForm(body, "init_image", new Blob([payload.ImageToImageRequestBody.init_image.content], {
            type: "application/octet-stream",
        }), payload.ImageToImageRequestBody.init_image.fileName);
    }
    appendForm(body, "text_prompts", encodeJSON("text_prompts", payload.ImageToImageRequestBody.text_prompts, {
        explode: true,
    }));
    ...
    if (payload.ImageToImageRequestBody.clip_guidance_preset !== undefined) {
        appendForm(body, "clip_guidance_preset", payload.ImageToImageRequestBody.clip_guidance_preset);
    }
    if (payload.ImageToImageRequestBody.extras !== undefined) {
        appendForm(body, "extras", encodeJSON("extras", payload.ImageToImageRequestBody.extras, {
            explode: true,
        }));
    }
    if (payload.ImageToImageRequestBody.image_strength !== undefined) {
        appendForm(body, "image_strength", payload.ImageToImageRequestBody.image_strength);
    }
    if (payload.ImageToImageRequestBody.init_image_mode !== undefined) {
        appendForm(body, "init_image_mode", payload.ImageToImageRequestBody.init_image_mode);
    }
    ...
    if (payload.ImageToImageRequestBody.step_schedule_end !== undefined) {
        appendForm(body, "step_schedule_end", payload.ImageToImageRequestBody.step_schedule_end);
    }
    if (payload.ImageToImageRequestBody.step_schedule_start !== undefined) {
        appendForm(body, "step_schedule_start", payload.ImageToImageRequestBody.step_schedule_start);
    }
    ...
    if (payload.ImageToImageRequestBody.style_preset !== undefined) {
        appendForm(body, "style_preset", payload.ImageToImageRequestBody.style_preset);
    }
    const pathParams = {
    ...
    const path = pathToFunc("/v1/generation/{engine_id}/image-to-image")(pathParams);
