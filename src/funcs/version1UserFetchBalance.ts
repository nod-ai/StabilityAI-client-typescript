 ...
 */
export function version1UserFetchBalance(
  client: StabilityAIClientCore,
  request: operations.FetchBalanceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.BalanceResponseBody,
    | errors.ErrorT
    | errors.ErrorT
    | APIError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.FetchBalanceRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.BalanceResponseBody$inboundSchema),
