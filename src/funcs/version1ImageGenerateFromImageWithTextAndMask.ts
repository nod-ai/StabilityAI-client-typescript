...
export enum GenerateFromImageWithTextAndMaskAcceptEnum {
 ...
 */
export function version1ImageGenerateFromImageWithTextAndMask(
  client: StabilityAIClientCore,
  request: operations.GenerateFromImageWithTextAndMaskRequest,
  options?: RequestOptions & {
    acceptHeaderOverride?: GenerateFromImageWithTextAndMaskAcceptEnum;
  },
): APIPromise<
  Result<
    operations.GenerateFromImageWithTextAndMaskResponse,
    | errors.ErrorT
    | errors.ErrorT
    | APIError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GenerateFromImageWithTextAndMaskRequest$outboundSchema.parse(
        value,
      ),
  ...
  if (isBlobLike(payload.MaskingRequestBody.init_image)) {
    ...
    const buffer = await readableStreamToArrayBuffer(
      payload.MaskingRequestBody.init_image.content,
    );
    ...
    appendForm(
      body,
      "init_image",
      new Blob([payload.MaskingRequestBody.init_image.content], {
  ...
  appendForm(
    body,
    "text_prompts",
    encodeJSON("text_prompts", payload.MaskingRequestBody.text_prompts, {
      explode: true,
    }),
  );
  ...
  if (payload.MaskingRequestBody.clip_guidance_preset !== undefined) {
    appendForm(
      ...
      payload.MaskingRequestBody.clip_guidance_preset,
    );
  }
  if (payload.MaskingRequestBody.extras !== undefined) {
    appendForm(
      body,
      "extras",
      encodeJSON("extras", payload.MaskingRequestBody.extras, {
        explode: true,
      }),
    );
  }
  if (payload.MaskingRequestBody.mask_image !== undefined) {
    if (isBlobLike(payload.MaskingRequestBody.mask_image)) {
      ...
      isReadableStream(payload.MaskingRequestBody.mask_image.content)
    ) {
      const buffer = await readableStreamToArrayBuffer(
        payload.MaskingRequestBody.mask_image.content,
      );
      const blob = new Blob([buffer], { type: "application/octet-stream" });
      ...
      appendForm(
        body,
        "mask_image",
        new Blob([payload.MaskingRequestBody.mask_image.content], {
          type: "application/octet-stream",
        }),
  ...
  const path = pathToFunc("/v1/generation/{engine_id}/image-to-image/masking")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.GenerateFromImageWithTextAndMaskResponse$inboundSchema,
